...
class GameMark extends CustomPainter {
  ...
  const GameMark.panel()
    : this(at: const [0.78, 0.55, 0.32], cornerRadius: 0.18, border: true);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < at.length; i++) {
      ...
      canvas.save();
      canvas.translate(size.width * (0.5 + (at[i] - 0.5) * scale), ground);
      Figure(
        height: size.height * bodyHeight * scale,
        color: i == at.length - 1
  ...
  @override
  bool shouldRepaint(GameMark oldDelegate) => false;
}
