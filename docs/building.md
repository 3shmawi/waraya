...
- **`lights` — النور.** جوّه المستطيل الظل **مش موجود**: مش صلب، مبيدوسش زرار
  ولا مفتاح، مبيقتلش، وبيترسم باهت. القرار بمركز جسم الظل، وعلى التكّة.
- **`delays` — ظل لكل رقم**، الأقرب الأول. `delaySeconds` لسه شغالة ومعناها
  `[x]`. الأبعد بيترسم أبهت لوحده. وخد بالك من ارتفاع الأبواب: الظلين
  بيتركبوا، فالوصول ٣٢٨ مش ٢٣٢.

وكل مرحلة بتطلّع `requires` لوحدها من اللي جواها (`toggles`،
`inverted-plates`، `lights`، `delays`)، فاللي عنده بيلد أقدم من المفردة بيرفض المرحلة بالاسم بدل
ما يلعبها ناقصة. الشرح الكامل في `docs/phase-6-vocabulary.md`.

- **`solution` و`wrongIdeas` — الحل المسجّل والفكرة الغلط**، جوه المرحلة
  نفسها. `s` ثواني، `x` الاتجاه (-1 / 1)، `j` نطة، `c` قعدة، واللي مش مضغوط
  مبيتكتبش: وقوف في مكانك هو `{"s": 1.2}`.

**والمرحلة متتحسبش خلصت** غير لما يبقى معاها الاتنين، وبيتشغّلوا على اللعبة
الحقيقية في تست — `test/level/levels_test.dart`. نفس الهارنس
(`lib/level/playthrough.dart`) هو اللي بيشغّلهم في التست وفي الكليبات وفي
بوابة القبول، فاللي بيعدّي عندك هو اللي هيعدّي على السيرفر.
